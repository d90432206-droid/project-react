...
function App() {
  ...
  const saveData = useCallback((updatedProjects: Project[], updatedLogs: Log[]) => {
    setIsLoading(true);
    GasService.saveData(updatedProjects, updatedLogs)
      .then(() => {
  ...
  const handleProjectSave = (p: Project, isNew: boolean) => {
    ...
    saveData(newProjects, logs);
  };

  const handleLogSubmit = (log: Partial<Log>) => {
    let newLogs = [...logs];
    if (log.logId) {
      const idx = newLogs.findIndex(l => l.logId === log.logId);
      if (idx !== -1) newLogs[idx] = log as Log;
    } else {
      newLogs.push({ ...log, logId: Date.now() } as Log);
    }
    saveData(projects, newLogs);
  };
